...
<style>
    /* Style global untuk semua input/select/textarea */
    .form-input {
        padding: 12px 16px;
    ...
    <div>
        ...
        <form action="{{ route('pengeluaran.store') }}" method="POST" style="display:flex;flex-direction:column;gap:16px;">
            ...
            <div>
                ...
                <select id="barang_id" name="barang_id" required class="form-input">
                    <option value="">-- Pilih Barang --</option>
                    @foreach ($barangs as $barang)
                        <option value="{{ $barang->id }}">{{ $barang->kode }} - {{ $barang->nama_barang }}</option>
                    @endforeach
                </select>
                @error('barang_id')
                    <p style="color:#cc0000;font-size:12px;margin-top:4px;">{{ $message }}</p>
            ...
            <div>
                ...
                <input type="number" id="jumlah" name="jumlah" min="1"
                       value="{{ old('jumlah') }}" required class="form-input">
                @error('jumlah')
                    <p style="color:#cc0000;font-size:12px;margin-top:4px;">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select id="alasan" name="alasan" required class="form-input">
                    ...
                    <option value="internal">Keperluan Internal</option>
